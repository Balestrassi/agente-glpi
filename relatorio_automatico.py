"""
Relatório Semanal Automático — GLPI
Calcula a semana anterior (seg-dom), busca chamados via API,
gera PDF e envia ao grupo Telegram.
...
OUTPUT   = f"Relatorio_{seg_anterior.strftime('%d_%m')}_{dom_anterior.strftime('%d_%m_%Y')}.pdf"
...
def buscar_tickets_periodo(tok):
    """Busca todos os tickets criados na semana anterior."""
    tickets = []
    offset  = 0
    while True:
        r = requests.get(
    ...
    print(f"  Tickets encontrados no período: {len(tickets)}")
    return tickets

def ultimo_tecnico(tok, tid, requester_id):
    """Data do último followup feito por não-solicitante (técnico)."""
    try:
...
def requester_numeric_id(tok, tid):
    """Retorna o ID numérico do solicitante sem expand_dropdowns."""
    try:
...
def produto(titulo):
    tl = titulo.lower()
    if "messianica" in tl or "messiânica" in tl: return "RecebeMais Messiânica"
    if "manserv" in tl:                           return "RecebeMais Manserv"
    if "recebemai" in tl:                         return "RecebeMais"
    return "Outros"
...
def on_page(canvas, doc):
    ...
    canvas.drawString(1.5*cm, h-0.85*cm, f"Suporte RM — Relatório Semanal | {PERIODO}")
    canvas.drawRightString(w-1.5*cm, h-0.85*cm, f"Gerado em {HOJE_STR}")
    canvas.setFillColor(AZUL_ESC)
    canvas.rect(0, 0, w, 0.8*cm, fill=1, stroke=0)
    canvas.setFont("Helvetica", 8)
    canvas.setFillColor(BRANCO)
    canvas.drawString(1.5*cm, 0.27*cm, "Dados extraídos do GLPI | Filtro: data de criação dentro da semana")
...
def on_first(canvas, doc):
    ...
    canvas.setFont("Helvetica", 8)
    canvas.setFillColor(BRANCO)
    canvas.drawString(1.5*cm, 0.27*cm, "Suporte RM — Relatório Semanal de Chamados GLPI")
    canvas.restoreState()

# ── Seções do PDF ────────────────────────────────────────────────────
def capa(story, total, taxa):
    story.append(Spacer(1, 3*cm))
    t = Table([[Paragraph("RELATÓRIO SEMANAL<br/>DE SUPORTE", TITULO_C)]],
    ...
    story.append(Paragraph(
        f"Chamados filtrados por <b>data de criação</b> entre "
...
def visao_geral(story, total, resol, taxa, por_status, por_prod, por_tipo_d, nao_resolvidos):
    ...
    story += section("Volume por Produto")
    prod_order = ["RecebeMais Manserv","RecebeMais Messiânica","RecebeMais","Outros"]
    rows_p = []
    for p in prod_order:
        qtd = por_prod.get(p, 0)
        if qtd == 0: continue
        pct = round(qtd/total*100)
        rows_p.append((p, str(qtd), f"{pct}%", "█"*int(pct/5)+"░"*(20-int(pct/5))))
    story.append(styled_tbl(["Produto","Qtd.","%","Proporção"], rows_p,
...
def tempo_medio(story, tempos, tp_prod_ciclo, tp_prod_tecnico, tp_tipo_ciclo, tp_tipo_tecnico,
    ...
    story += section("Tempo Médio por Produto")
    rows_p = []
    for prod in ["RecebeMais Manserv","RecebeMais Messiânica","RecebeMais","Outros"]:
        if prod not in TEMPO_PROD_TECNICO: continue
    ...
    story.append(styled_tbl(
        ["Produto","Tempo Técnico","Tempo Total","Qtd."],
...
def enviar_telegram(pdf_bytes):
    ...
    r = requests.post(
        ...
        data={
            ...
            "caption": (
                f"📊 *Relatório Semanal de Suporte*\n"
                f"Período: {PERIODO}\n"
...
# ── Main ──────────────────────────────────────────────────────────────
def main():
    print(f"[{HOJE_STR}] Relatório Automático — período: {PERIODO}")
    ...
    if not tickets_raw:
        print("  Nenhum ticket encontrado no período. Encerrando.")
        return

    # Montar estrutura compatível com relatorio_semana.py
    TICKETS = []
    for t in tickets_raw:
        ...
        urg        = t.get("urgency", 3)
        st         = t.get("status", 1)
        TICKETS.append((tid, titulo, criacao, atualizacao, urg, st))

    # Calcular ultimo_tecnico para tickets resolvidos/fechados
    print(f"  Buscando último comentário técnico para tickets resolvidos...")
    tok2 = get_session()
    try:
        DADOS = {}
        resolvidos = [(tid, titulo, criacao, atu, urg, st)
                      for (tid, titulo, criacao, atu, urg, st) in TICKETS if st in (5, 6)]
        for i, (tid, _, criacao, _, _, _) in enumerate(resolvidos):
    ...
    for tid, titulo, criacao, atualizacao, urg, st in TICKETS:
